...
function statusConnect(client) {
  client.on("statusUpdate", function (status) {
    document.getElementById(
      "debug-console"
    ).value += `Trạng thái tài khoản đã thay đổi thành ${status} \n`;
  });
}
  ...
  client.on("invite", async (session) => {
    try {
      ...
      document.getElementById("callIn-info").innerHTML =
        "Cuộc gọi đến từ" + "\xa0" + session.phoneNumber;
      document.getElementById("debug-console").value +=
        `Call incoming` + "\xa0" + session.phoneNumber + "\n";
      sessionCall = session;
      statusCall(session);
...
function statusCall(session) {
  session.on("statusUpdate", function (status) {
    document.getElementById("debug-console").value +=
      `Trạng thái của cuộc gọi đã thay đổi thành ${status.status}` + "\n";
  });
}
...
document
  .getElementById("accecpt-call-btn")
...
document
  ...
  .addEventListener("click", RejectIncomingCall);
...
document
  .getElementById("terminate-call-btn")
...
document
  ...
  .addEventListener("click", CancelOutGoingCall);
...
document
  .getElementById("unhold-call-btn")
  .addEventListener("click", UnholdCall);
...
function changeInputVolume(val) {
  ...
  document.getElementById(
    "debug-console"
  ).value += `Volume input change to ${val} \n`;
}

document
  .getElementById("volume-range-input")
  .addEventListener("change", function () {
    changeInputVolume(document.getElementById("volume-range-input").value);
  });
...
function changeOutputVolume(val) {
  ...
  document.getElementById(
    "debug-console"
  ).value += `Volume output change to ${val} \n`;
}

document
  .getElementById("volume-range-output")
  .addEventListener("change", function () {
    changeOutputVolume(document.getElementById("volume-range-output").value);
  });
...
function selectDeviceInput() {
  ...
  if (!navigator.mediaDevices?.enumerateDevices) {
    ...
    navigator.mediaDevices
      .enumerateDevices()
      .then((devices) => {
        var devicesInput = devices.filter(
        ...
        document
          .getElementById("selectDeviceInput")
          .addEventListener("change", function () {
            changeInputDevice(
              document.getElementById("selectDeviceInput").value,
              document.getElementById("selectDeviceInput").options[
                document.getElementById("selectDeviceInput").selectedIndex
              ].text
            );
          });
      })
...
function selectDeviceOutput() {
  ...
  if (!navigator.mediaDevices?.enumerateDevices) {
    ...
    navigator.mediaDevices
      .enumerateDevices()
      .then((devices) => {
        var devicesOutput = devices.filter(
        ...
        document
          .getElementById("selectDeviceOutput")
          .addEventListener("change", function () {
            changeOutputDevice(
              document.getElementById("selectDeviceOutput").value,
              document.getElementById("selectDeviceOutput").options[
                document.getElementById("selectDeviceOutput").selectedIndex
              ].text
            );
          });
      })
...
document.getElementById("typeCall").addEventListener("change", function () {
  changeTypeCall(document.getElementById("typeCall").value);
});
